...
;;
;; Ported from cloud-itonami-assoc-9411-prt-cip (which took it from -gbr-cbi),
;; with the date reader rewritten for how THESE sources write dates. Two are
;; prose in two languages -- bdi.eu serves the same history in English and
;; German ("On 19 October 1949", "Am 19. Oktober 1949") -- and one is a German
;; administrative register that writes 16.02.2022 and no month name at all.
;; That last case gets its own :date-basis rather than being squeezed into the
;; word matcher, because the two failure modes are opposite: the word matcher
;; would reject a span that plainly carries the date, and the day matcher alone
;; would accept any stray 02 elsewhere on the page as February.
;;
...
(def provenance->host
  {:official-bdi-eu          "bdi.eu"
   :german-lobbyregister     "lobbyregister.bundestag.de"
...
;; bdi.eu serves this catalog in both languages and de.wikipedia.org only in
;; German, so a month can arrive as "October" or as "Oktober" ("Am 19. Oktober
;; 1949"). German always capitalises month names; matching is done lowercased.
(def de-months ["januar" "februar" "märz" "april" "mai" "juni"
...
(defn- month-word-in?
  "Does `hay` name month `mm` in words, in either language?
   Numeric German dates are NOT read here -- see :in-quote-numeric-dmy, which
   requires the whole DD.MM.YYYY to stand in the span rather than letting a
     ...
     (fn [e]
       (let [id (:association-rule/id e)
             ...
             date-hay (case basis
                        ...
                        :in-quote-numeric-dmy q
                        :corroborating-quote cq
                        nil)
             ;; A German administrative register writes 16.02.2022, not
             ;; "16 February 2022". Reading month "02" with the word matcher
             ;; would fail on a span that plainly carries the date; reading it
             ;; with `day-in?` alone would let any stray 02 on the page stand
             ;; in for February. So this basis is checked as one literal.
             numeric-dmy? (= :in-quote-numeric-dmy basis)
         ...
         (concat
          ...
          (when (and numeric-dmy? date-hay)
            (cond
              (not (and m d))
              [[:numeric-dmy-without-full-date
                (str id ": :date-basis is :in-quote-numeric-dmy but " date
                     " is not a full YYYY-MM-DD, so there is no DD.MM.YYYY to look for")]]
              (not (str/includes? date-hay (str d "." m "." y)))
              [[:numeric-date-not-in-cited-span
                (str id ": records " date " with :date-basis " basis ", but the cited "
                     "span does not contain " (str d "." m "." y))]]))
          (when (and m date-hay (not numeric-dmy?) (not (month-word-in? date-hay m)))
            [[:month-not-in-cited-span
              (str id ": records month " m " of " date " but the cited span names no "
                   "such month")]])
          (when (and d date-hay (not numeric-dmy?) (not (day-in? date-hay d)))
